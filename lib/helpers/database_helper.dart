...
''');
  ...
  //   log("[DB] Inserting slot: ${entry.slot.label}, amount: ${entry.targetIntake} mL");
  //   await db.insert(
  //     'hydration_slots',
  //     {
  ...
  Future<void> insertOrUpdateSlot(HydrationEntry entry,
    ...
    log("[DB] Inserting slot: ${entry.slot.label}, "
        "targetIntake: ${entry.targetIntake} mL, "
        "waterDrank: ${entry.waterDrank}");
    ...
    await db.insert(
      'hydration_slots',
      {
        ...
        /// Store BLE target intake correctly
        'waterGoal': entry.targetIntake,

        /// Store consumed water
        'waterDrank': entry.waterDrank,

        /// Save status correctly
        'status': entry.status.toString().split('.').last,
      },
      conflictAlgorithm: ConflictAlgorithm.replace,
    );
    ...
    log("[DB] Current slots in DB:");
    for (var s in allSlots) {
      log("  ${s['slotName']} - targetIntake(waterGoal): ${s['waterGoal']}, "
          "waterDrank: ${s['waterDrank']}, status: ${s['status']}");
    }
  }
  ...
  //       targetIntake: (row['waterGoal'] as num).toDouble(),
  ...
  Future<List<HydrationEntry>> getAllSlots() async {
    ...
    return List.generate(maps.length, (i) {
      ...
      return HydrationEntry(
        ...
        startTime: _epochToTimeOfDay(row['startEpoch'] as int),
        endTime: _epochToTimeOfDay(row['endEpoch'] as int),
        targetIntake: (row['waterGoal'] as num).toDouble(), // FIXED
        waterDrank: (row['waterDrank'] as num?)?.toDouble() ?? 0.0,
        status: row['status'] == 'completed'
            ? HydrationStatus.completed
            : HydrationStatus.pending,
